...
$(document).on('click', '.view_request',function() {
    ...
    $.ajax({
      url:admin_url+'service_requests/get_details',
      data:{ record_id : record_id },
      type:'post',
      dataType: 'json',
      success:function(res) {
...
$(document).on('click', '#assign_request_btn',function() {

    if($('#employee_id').val() == "") {
        swal("Error", "Please select a employee." , "error");
        return false;
    }

    var formdata = $("#assign_request_form").serialize();

    $.ajax({
      url:admin_url+'service_requests/assign_request',
      data:formdata,
      type:'post',
      dataType: 'json',
      success:function(status) {
...
$(document).on('click', '.view_ticket',function() {
    ...
    $.ajax({
      url:admin_url+'tickets/get_details',
      data:{ record_id : record_id },
      type:'post',
      dataType: 'json',
      success:function(res) {
...
$(document).on('click', '#assign_ticket_btn',function() {
    ...
    var formdata = $("#assign_ticket_form").serialize();
